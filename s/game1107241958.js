window.gnmathserver="fgdsdfgsdfg-production.up.railway.app"
var testing = false;
...
if (!ggl) {
    ...
    sesti.src = "s2/sest5.png"
}
if (ggl)(async () => {
    sestt = await PIXI.Assets.load("s2/sest5.png")
})();
...
var redraw = function () {
            ...
            for (var i = foods_c - 1; i >= 0; i--) {
                ...
                if (fo.rx >=
                    ...
                    if (fo.rad == 1) {
                        ...
                        ctx.globalAlpha = ba * (.5 + .5 * Math.cos(fo.gfr / 13)) * fo.fr;
                        ctx.drawImage(fo.fi,
                            0, 0, fo.fw, fo.fh, fx, fy, fo.fw * fo.rad, fo.fh * fo.rad)
                    }
            }
        } else
            for (var i = foods_c - 1; i >= 0; i--) {
        ...
        if (o.iiv) {
                ...
                for (var j = o.pts.length - 1; j >= 0; j--) {
                    ...
                    if (rl > 0) {
                        ...
                        if (!dfl) {
                            ...
                            dfl = true
                        } else ctx.quadraticCurveTo(mww2 +
                            (lpx - view_xx) * gsc, mhh2 + (lpy - view_yy) * gsc, mww2 + (ax - view_xx) * gsc, mhh2 + (ay - view_yy) * gsc)
                    }
                }
                ctx.save();
                ctx.lineJoin = "round";
                        ...
                        for (j = bp - 1; j >= 0; j--)
                            if (pbu[j] >= 1) {
                                ...
                                if (j < 4) {
                                    ...
                                    -gsc * lsz, -gsc * lsz, gsc * 2 * lsz, gsc * 2 * lsz);
                                if (fdhc && j < fdl) {
                                    fdam = 1 - j / fdl;
                            ...
                            if (high_quality || gla > 0) {
                                ...
                                for (j = bp - 1; j >= 0; j--)
                                    if (pbu[j] == 2) {
                                        ...
                                        if (j < 4) {
                                            var tsz = glsz * (1 + (4 - j) * o.swell);
                                            ctx.drawImage(per_color_imgs[rbcs[j % rbl]].kfmc, -gsc * tsz, -gsc * tsz, gsc * 2 * tsz, gsc *
                                                2 * tsz)
                        ...
                        if (high_quality || gla > 0) {
                            ...
                            for (j = bp - 1; j >= 0; j--)
                                if (pbu[j] == 2) {
                                    ...
                                    if (j < 4) {
                                        var tsz = glsz * (1 + (4 - j) * o.swell);
                                        ctx.drawImage(opci.kfmc,
                                            -gsc * tsz, -gsc * tsz, gsc * 2 * tsz, gsc * 2 * tsz)
                    ...
                    if (o.dead) {
                        ...
                        for (j = bp - 1; j >= 0; j--)
                            if (pbu[j] == 2) {
                                ...
                                } else ctx.drawImage(kdmc, -dsz, -dsz,
                                    2 * dsz, 2 * dsz);
                                ctx.restore()
                            } ctx.restore()
                    } ctx.restore()
            ...
            if (o.one_eye) {
                ...
                if (ggl) {
                    ...
                    ctx.drawImage(o.ebi, 0, 0, o.ebiw, o.ebih, mww2 + (ex + hx - esz / 2 - view_xx) * gsc, mhh2 + (ey + hy - esz / 2 - view_yy) * gsc, esz * gsc,
                        esz * gsc)
                }
    ...
    if (ggl) {
            ...
            for (var i = foods_c - 1; i >= 0; i--) {
                ...
                if (fo.rx >= fpx1 && fo.ry >= fpy1 && fo.rx <= fpx2 && fo.ry <= fpy2) {
                    ...
                    if (fo.rad == 1) {
                        ...
                        ctx.globalAlpha = fal * (.5 + .5 * Math.cos(fo.gfr / 13)) *
                            fo.fr;
                        ctx.drawImage(fo.gfi, fx, fy)
                    } else {
                        fx -= fo.gfw2 * fo.rad;
        ...
        for (var i = preys.length - 1; i >= 0; i--) {
            ...
            if (pr.rad != 1) pal *= Math.pow(pr.rad,
                .25);
            trd = pr.rad * 2;
            px = px * gsc + mww2;
            py = py * gsc + mhh2;
...
function connect() {
    ...
    if (!forcing) {
        ...
        if (fbso != null) {
            ...
            let tmpbso = sos[Math.floor(Math.random() *
                sos.length)];
            bso = tmpbso
            window.bso = tmpbso;
    ...
    if (!playing)
        if (dead_mtm == -1) {
            ...
            if (!ever_showed_entercode) {
                ...
                for (var i = etcobs.length - 1; i >= 0; i--)
                    if (i == 10) etcobs[i].ii.src = "s/codedel.png";
                    else etcobs[i].ii.src = "s/code" + i + ".png";
            ...
            for (j = 2; j < pts.length; j += 4) {
                ...
                for (var k = 1; k <= kc; k++) {
                    ...
                    var fsz = sz * 1.15 * Math.min(1,
                        lgsc * (.2 + .8 * lga) * (30 * lgss * r - q / 20 - i / 2));
                    if (fsz > .5) {
...
svitl.textContent = "Only 1 server because im too lazy to proxy the rest";
